...
tf.app.flags.DEFINE_string("train_dir", "/tmp/train_seq2seq",
                           "")
tf.app.flags.DEFINE_integer("input_length", 31,
                            "")
tf.app.flags.DEFINE_integer("train_length", 30,
                            "")
tf.app.flags.DEFINE_integer("test_length", 31,
                            "")
...
def train():
    ...
    with tf.Graph().as_default() as g:
        ...
        for length_idx in xrange(FLAGS.input_length):
            ...
            if length_idx < FLAGS.train_length:
                weight = np.ones(FLAGS.batch_size, dtype=np.float32)
            else:
        ...
        with tf.variable_scope('lab4'):
            ...
            decoder_outputs_test, _ = tf.contrib.legacy_seq2seq.embedding_rnn_seq2seq(
                encoder_inputs=encoder_inputs,
                decoder_inputs=decoder_inputs,
                cell=cell,
                num_encoder_symbols=FLAGS.word_size,
        ...
        def accuracy(dec_outputs_batch, labels):
            anss = [tf.argmax(logits_t, axis=1)
                    for logits_t in dec_outputs_batch]

            table = tf.equal(tf.to_int32(anss, name='ToInt32'),
                             labels, name="equal")
        ...
        acc, ans = accuracy(decoder_outputs_test, labels)
